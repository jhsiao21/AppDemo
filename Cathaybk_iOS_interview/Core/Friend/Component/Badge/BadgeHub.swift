...
public class BadgeHub: NSObject {
    ...
    private var badgeColor = UIColor.softPink
    private var curOrderMagnitude: Int = 0
    private var badgeCircle: BadgeView!
    ...
    private struct Constants {
        static let notificHubDefaultDiameter: CGFloat = 18
...
//            countLabel?.text = (count > 99) ? "99+" : "\(count)"
    ...
    public func setView(_ view: UIView?, andCount startCount: Int) {
        ...
        badgeCircle = BadgeView()
        badgeCircle?.isUserInteractionEnabled = false
        badgeCircle.backgroundColor = badgeColor
        
        countLabel = UILabel(frame: badgeCircle.frame)
        ...
        let atFrame = CGRect(x: 43 - ((Constants.notificHubDefaultDiameter) * 2 / 3),
        ...
        view?.addSubview(badgeCircle)
        view?.addSubview(countLabel!)
        view?.bringSubviewToFront(badgeCircle)
    ...
    public func setCircleAtFrame(_ frame: CGRect) {
        badgeCircle.frame = frame
        baseFrame = frame
        initialFrame = frame
        
        countLabel?.frame = badgeCircle.frame
        badgeCircle.layer.cornerRadius = frame.size.height / 2
        countLabel?.font = UIFont.systemFont(ofSize: frame.size.width / 2)
    }
            
    public func moveCircleBy(x: CGFloat, y: CGFloat) {
        var frame: CGRect = badgeCircle.frame
        frame.origin.x += x
        frame.origin.y += y
...
//        countLabel?.text = "\(count)"
    ...
    func resizeToFitDigits() {
        ...
        orderOfMagnitude = (orderOfMagnitude >= 2) ? 2 : 1
...
//        print("frame after: \(frame)")
    ...
    func bumpCenterY(yVal: CGFloat) {
        var center: CGPoint = badgeCircle.center
        center.y = initialCenter.y - yVal
        badgeCircle.center = center
    ...
    public func pop() {
        ...
        badgeCircle.layer.add(group, forKey: nil)
        
        UIView.animate(withDuration: TimeInterval(timeStart), animations: {
            var frame: CGRect = self.badgeCircle.frame
            let center: CGPoint = self.badgeCircle.center
            frame.size.height = CGFloat(popStartHeight)
            frame.size.width = CGFloat(popStartWidth)
            self.badgeCircle.frame = frame
            self.badgeCircle.center = center
        }) { complete in
    ...
    public func bump() {
        if !initialCenter.equalTo(badgeCircle.center) {
